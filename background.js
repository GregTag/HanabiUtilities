...
class PreGameNotification {
    ...
    async shouldNotify(utilities) {
        ...
        const tabActiveAndFocused = await utilities.isTabActiveAndFocused();
        const onPreGamePage = utilities.isOnPreGamePage(this.data.tableId);
        if (tabActiveAndFocused && onPreGamePage) {
            return false;
        }

        const settings = await utilities.getSettings();
        if (!settings.preGameNotifications || this.data.players.length === 0) {
            utilities.joinedTableData = null;
            return false;
        }
        ...
        this.playersChanges = PreGameNotification.detectPlayerChanges(this.data.players, previousData);
